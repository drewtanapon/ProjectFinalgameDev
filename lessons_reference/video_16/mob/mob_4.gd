...
@onready var Warrior_model = %Skeleton_Warrior
...
@onready var hitbox: Area3D = $"Skeleton_Warrior/Hitbox"    # ใน mob.tscn ควรมีลูกเป็น Area3D ชื่อ Hitbox พร้อม CollisionShape3D
...
func _physics_process(delta):
	...
	Warrior_model.rotation.y = Vector3.FORWARD.signed_angle_to(direction, Vector3.UP) + PI
...
func take_damage():
	...
	if health == 0:
		ko_sound.play()
		Warrior_model.animation_tree.active = false
		Warrior_model.die()
